...
- **For Redis OSS**: emit a full `ACL SETUSER <user> on ><changeme> <rule>` command — directly runnable via `redis-cli`. Replace `<replace_password_here>` with the actual credential before running.
