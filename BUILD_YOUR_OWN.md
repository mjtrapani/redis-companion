...
|-----------|-----|---------------|
| **Skill** | Owns the entry point. A skill can act as the **orchestrator** (slash command + `AskUserQuestion` + agent dispatch — where the workflow lives) and/or as **auto-loaded reference content** (syntax, mappings, version deltas). This plugin has one of each: `rule` is the orchestrator; `acl-reference` is the knowledge base. | Reusable across agents and sessions. Replacing the reference doc is a content change, not code. |
