...
- **`ACL LOG`-driven denial diagnosis (inverse mode).** Instead of generating an ACL from code, read the audit trail of recent ACL denials on a live Redis, group them, and suggest minimal additions to unblock the application. Requires the same MCP ACL surface as the live-apply loop above.
