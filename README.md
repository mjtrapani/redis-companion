...
- **ACL selectors (Redis 7+) for finer per-key-pattern grants.** Today the plugin emits flat rules. With selectors, individual commands could be scoped to specific key families — e.g., `+SET` granted only on `cache:*`, `+XADD` only on `stream:logs:*`. Models real "this command writes only to this key family" intent precisely. v1 deliberately emits flat rules for explainability; selectors are an opt-in tightening.
