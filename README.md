...
**Cross-version category deltas aren't programmatically derived.** The `command-category-map.md` reference is canonical for one Redis version (`redis/redis@8.6.3`, pulled from upstream `src/commands/*.json`). The cross-version differences — e.g., `EVAL` moved from `@write` to `@scripting` in Redis 7, modules joined `@read`/`@write` in Redis 8 — live in a separate `version-deltas.md` file that was hand-curated based on training-data knowledge of Redis ACL history, not formally verified against multi-version source. The major-version shifts are well-known Redis evolution and should be reliable; specific minor-version cutoffs (e.g., *"selectors landed in 7.2"*) are vulnerable to small errors. The fix is on the roadmap — see [`docs/FUTURE_WORK.md`](./docs/FUTURE_WORK.md).

The agent **flags but doesn't silently bake** these:

- **Client method → Redis command mappings** for non-CRUD usage (scripting helpers, locks, subcommand-named methods, transactional pipelines, Sentinel/Cluster client mode, sharded pub/sub). The convention "method name = command name" holds for ~90% of calls but breaks for the rest. For ambiguous mappings the agent makes one targeted `WebFetch` to the library's official API docs; if that doesn't resolve the ambiguity, the call is flagged in the agent's output for review rather than baked into the rule. *(MCP-driven `MONITOR` verification against a live test instance is on the roadmap — see [`docs/FUTURE_WORK.md`](./docs/FUTURE_WORK.md).)*
- **Inferred grants from comments** (e.g., a `# TODO: add scripting` near pubsub code). Surfaced as a question; never baked in.

## What's next

The biggest future-work items, in rough priority order. The exhaustive 16-item list with detailed rationale for each item lives in [`docs/FUTURE_WORK.md`](./docs/FUTURE_WORK.md).
...
- **Expand client library coverage + ground the mappings in authoritative sources.** v1 covers `redis-py`, `ioredis`, and `go-redis` only. Adding Java (`jedis`/`lettuce`), .NET (`StackExchange.Redis`), Ruby (`redis-rb`), PHP (`predis`/`phpredis`), and Rust (`redis-rs`/`fred`) would broaden the plugin's reach significantly. The existing mappings were derived from training data + published API docs, not first-party source-code review. A more rigorous approach would parse each library's source (or its type stubs / generated API reference) programmatically and verify against `MONITOR` traces — replacing the "method name = command name" convention plus hand-curated caveats with derivable mappings.
- **Runtime-observed mode** via `MONITOR` + `ACL LOG`. v1 infers access patterns by reading source code, which has real limits — dynamic key construction, conditional command paths, scripting helpers, and library-internal commands can all be missed or mis-estimated. A runtime-observed mode would capture what the service *actually* does: record the live command stream via `MONITOR` while typical workloads run, and read `ACL LOG` for any `NOPERM` denials against an existing rule. The agent derives the access pattern from observed usage rather than just code-reading. Generalizes `ACL LOG`-driven denial diagnosis (item #6 in `docs/FUTURE_WORK.md`) into a primary-source workflow.
...
The plugin's premise — the same pain point of constructing custom Redis ACLs from intent — is inspired by [Redis ACL Builder](https://github.com/markotrapani/redis-acl-builder), a manual GUI tool I built earlier. The command-category reference data in *this* plugin was rebuilt from scratch by pulling `commands.json` directly from upstream `redis/redis@8.6.3` — not reused from the earlier project.
